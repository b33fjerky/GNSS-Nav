...
int main(int argc, char **argv)
{
	...
	unsigned char buf[31];										// Buffer for data being read/ written on the i2c bus
	...
	if (read(fd, buf, 31) != 31) {								// Read back data into buf[]
		...
		int calstatus = buf[30];
		int cel = buf[25];
		printf("Calibration Status [0-3]: %d\n", calstatus);
		printf("Temprature in celcius: %d\n", cel);
